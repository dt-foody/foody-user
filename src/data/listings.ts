...
import __foodListing from "./jsons/__foodListing.json";
...
import {
  ...
  DEMO_FOOD_CATEGORIES,
} from "./taxonomies";
import { CarDataType, ExperiencesDataType, StayDataType, FoodDataType  } from "./types";
...
const DEMO_FOOD_LISTINGS: FoodDataType[] = __foodListing.map(
  (post, index) => {
    // Lấy category theo id
    const category = DEMO_FOOD_CATEGORIES.find(
      (taxonomy) => taxonomy.id === post.foodCategoryId
    );

    return {
      ...post,
      id: `foodListing_${index}_`,
      saleOff: !index ? "-20% today" : post.saleOff,
      isAds: !index ? true : post.isAds,
...
export { DEMO_STAY_LISTINGS, DEMO_EXPERIENCES_LISTINGS, DEMO_CAR_LISTINGS, DEMO_FOOD_LISTINGS };
